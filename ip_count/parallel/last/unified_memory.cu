...
//
//    int * h_spectra, * h_peptides, * h_inner_product;
//    cudaMallocManaged(&h_spectra, SIZE_SPECTRA * sizeof(int));
//    cudaMallocManaged(&h_peptides, SIZE_PEPTIDES * sizeof(int));
//    cudaMallocManaged(&h_inner_product, SIZE_INNER_PRODUCT * sizeof(int));
//
//    auto malloc_host_end = std::chrono::steady_clock::now();
//    time_res << "Malloc Managed: " << std::chrono::duration_cast<std::chrono::milliseconds>(malloc_host_end - malloc_host_beg).count() << std::endl;
//
...
//    kernel <<<GRID_SIZE, BLOCK_SIZE >>> (h_peptides, h_inner_product, MAX_LENGTH_PEPTIDE);

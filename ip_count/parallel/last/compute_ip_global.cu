...
__global__ void comp_ip_kernel(int* peptides, int* spectra, int* inner_product) {
    int result_ip = 0;
    ...
    for (size_t i = 0, j = 0; i < MAX_LENGTH_PEPTIDE; ++i, ++j) {
        ...
        if (peptide_val != -1) {
            result_ip += spectra[peptide_val];
        }
    }
    inner_product[current_peptide] = result_ip;
}

void comp_ip(int* peptides, int* spectra, int* inner_product, int num_of_peptides) {
    dim3 blockSize(BLOCK_SIZE, 1, 1);
    dim3 gridSize(num_of_peptides / BLOCK_SIZE, 1, 1);

    comp_ip_kernel <<<gridSize, blockSize>>> (peptides, spectra, inner_product);
}
...
int main()
{
    while (cin) {
        ...
        comp_ip(d_peptides.getData(), d_spectra.getData(), d_inner_product.getData(), NUM_OF_PEPTIDES);
